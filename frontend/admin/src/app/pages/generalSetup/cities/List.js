...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('cities/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('cities', {records:'all'}).then(response => {
      let related_models = ['state']
      ...
      document.getElementById('cities-csv-btn').click();
    })
  }
        ...
        <div className="kt-portlet__head kt-portlet__head--lg">
            <div className="kt-portlet__head-label">
              <span className="kt-portlet__head-icon">
                <i className="kt-font-brand flaticon2-open-box" />
              </span>
                {numSelected > 0 ? (
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/cities/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
                                  ...
                                  <ul className="kt-nav">
                                    ...
                                    <li className="kt-nav__item">
                                      <a className="kt-nav__link">
                                        <i className="kt-nav__link-icon la la-upload" />
                                        <CSVReader inputId="cities-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbCities').then((response)=>{
...
function parseRowData(rowData) {
    ...
    if( Array.isArray(rowData) && rowData.length > 0){
      rowData.forEach(function(item, index){
        if(item.state)
          rowData[index]['state__name'] = item.state.name;
        else
          rowData[index]['state__name'] = '';
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abcities'),
    edit : userHasPermission('change_abcities'),
    delete : userHasPermission('delete_abcities'),
    view : userHasPermission('view_abcities'),
  });
  let history = useHistory();
  
  React.useEffect(() => {
		if($.fn.dataTable.isDataTable( '#cities' )) {
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#cities' ) ) {
    ...
    function loadDatatable() {
      let url = API_URL+'cities?format=datatables';
      let columnDefs = [
        ...
        },
        {
          ...
          "createdCell": function (td, cellData, rowData, row, col) {
            ReactDOM.render(
            <a onClick={() => history.push("/admin/cities/"+rowData.id) }>
            {cellData}
            </a>
                , td
            );
        ...
        columnDefs.push(
            {
              ...
              "createdCell": function (td, cellData, rowData, row, col) {
                ...
                {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/cities/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
      ...
      $('#cities').DataTable({
        ...
        serverSide:true,
        displayStart : getPageNum('cities', props),
        ajax: {
          url:url ,
          method: "GET",
      ...
      $('#cities').on( 'page.dt', function () {
        setPageNum($('#cities'), 'cities', props);
      });
    }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('cities/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('cities/'+rows[userIndex].id+'/').then(function (response) {
				let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id="cities">
                      <thead>
                      <tr>
                          ...
                          <th data-data="state__name">State</th>
