...
import { list, patch, del, userHasPermission, MESSAGE_TYPES, importData, API_URL} from "../../../crud/api";
...
const EnhancedTableToolbar = props => {
  ...
  const { numSelected, selected, setSelected, setSelectedMany, setRowsData, rows, setTemp, csvData } = props;


  function handleDelAll(event) {
    if(selected.length) {
      let data = {ids:selected}
      del('emails/'+selected[0]+'/', data).then(function (response) {
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/email-template/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
function updateTable(newData, type) {
    const table = $('#email-templates').DataTable();
...
function parseRowData(rowData) {
    if( Array.isArray(rowData) && rowData.length > 0){
      rowData.forEach(function(item, index){
        MESSAGE_TYPES.map(message_type => {
          if(message_type.value == item.message_type)
            rowData[index]['message_type'] = message_type.label;
        })
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abcannedemails'),
    edit : userHasPermission('change_abcannedemails'),
    delete : userHasPermission('delete_abcannedemails'),
    view : userHasPermission('view_abcannedemails'),
  });
  let history = useHistory();
  
  React.useEffect(() => {
    if($.fn.dataTable.isDataTable( '#email-templates' )) {
      let table = $('#email-templates').DataTable();
      table.destroy();
      setTable(true);
    }
    return () => {
          setRowsData([]);
          let dtable = $('#email-templates').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
}, []);
...
React.useEffect(() => {
  if (table && $.fn.dataTable.isDataTable( '#email-templates' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'emails?format=datatables';
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
          ReactDOM.render(
          <a onClick={() => history.push("/admin/email-template/"+rowData.id) }>
          {cellData}
          </a>
              , td
          );
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/email-template/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#email-templates').DataTable({
      ...
      serverSide:true,
      displayStart : getPageNum('emails', props), 
      ajax: {
        url:url ,
        method: "GET",
    ...
    $('#email-templates').on( 'page.dt', function () {             
      setPageNum($('#email-templates'), 'emails', props);
    });                                           
  }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('emails/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('emails/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
  ...
  return (
    ...
    <div className={classes.root}>
      ...
      <Paper className={classes.paper}>
        ...
        <EnhancedTableToolbar csvData={csvData} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} />
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"email-templates"}>
                      <thead>
                      <tr>
                          ...
                          <th data-data="message_type">Message Type</th>
