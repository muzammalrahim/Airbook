...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('states/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('states', {records:'all'}).then(response => {
      ...
      document.getElementById('states-csv-btn').click();
    })
  }
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/states/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
                                  ...
                                  <ul className="kt-nav">
                                    ...
                                    <li className="kt-nav__item">
                                      <a className="kt-nav__link">
                                        <i className="kt-nav__link-icon la la-upload" />
                                        <CSVReader inputId="states-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbStates').then((response)=>{
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abstates'),
    edit : userHasPermission('change_abstates'),
    delete : userHasPermission('delete_abstates'),
    view : userHasPermission('view_abstates'),
  });
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#state' ) ) {
    ...
    function loadDatatable() {
      let url = API_URL+'states?format=datatables';
      let columnDefs = [
        ...
        },
        {
          ...
          "createdCell": function (td, cellData, rowData, row, col) {
            ReactDOM.render(
            <a onClick={() => history.push("/admin/states/"+rowData.id) }>
            {cellData}
            </a>
                , td
            );
        ...
        columnDefs.push(
            {
              ...
              "createdCell": function (td, cellData, rowData, row, col) {
                ...
                {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/states/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
      ...
      $('#state').DataTable({
        ...
        serverSide:true,
        displayStart : getPageNum('states', props),
        ajax: {
          url:url ,
          method: "GET",
      ...
      $('#state').on( 'page.dt', function () {             
        setPageNum($('#state'), 'states', props);
      }); 
    }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('states/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('states/'+rows[userIndex].id+'/').then(function (response) {
				let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"state"}>
                      <thead>
                      <tr>
