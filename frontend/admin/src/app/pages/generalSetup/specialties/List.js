...
const EnhancedTableToolbar = props => {
	...
	const { numSelected, type, selected, setSelected, setSelectedMany, setRowsData, rows, setTemp, csvData } = props;


	function handleDelAll(event) {
		if(selected.length) {
			let data = {ids:selected}
			del('speciality/'+selected[0]+'/', data).then(function (response) {
								...
								<div className="kt-portlet__head-actions">
											{numSelected > 0 ? (
													...
													<div className="btn-group">
														<Link to={"/admin/speciality/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
function updateTable(newData, type) {
		const table = $('#specialities').DataTable();
...
export default function EnhancedTable(props) {
	...
	const [permission, setPermission] = React.useState({
		create : userHasPermission('add_abspecialities'),
		edit : userHasPermission('change_abspecialities'),
		delete : userHasPermission('delete_abspecialities'),
		view : userHasPermission('view_abspecialities'),
	});
	let history = useHistory();
	
	React.useEffect(() => {
		if($.fn.dataTable.isDataTable( '#specialities' )) {
			let table = $('#specialities').DataTable();
			table.destroy();
			setTable(true);
		}
		return () => {
          setRowsData([]);
          let dtable = $('#specialities').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
	}, []);
	...
	React.useEffect(() => {
	if (table && $.fn.dataTable.isDataTable( '#specialities' ) ) {
	...
	function loadDatatable() {
		let url = API_URL+'speciality?format=datatables';
		let columnDefs = [
			...
			},
			{
				...
				"createdCell": function (td, cellData, rowData, row, col) {
					ReactDOM.render(
					<a onClick={() => history.push("/admin/speciality/"+rowData.id) }>
					{cellData}
					</a>
							, td
					);
			...
			columnDefs.push(
					{
						...
						"createdCell": function (td, cellData, rowData, row, col) {
							...
							{permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/speciality/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
		...
		$('#specialities').DataTable({
			...
			serverSide:true,
			displayStart : getPageNum('speciality', props), 
			ajax: {
				url:url ,
				method: "GET",
		...
		$('#specialities').on( 'page.dt', function () {             
			setPageNum($('#specialities'), 'speciality', props);
		});  
	}
	...
	function confirm(event) {
		if(action == 'status') {
			...
			patch('speciality/'+rows[userIndex].id+'/', data).then(function (response) {
			...
			del('speciality/'+rows[userIndex].id+'/').then(function (response) {
				let rowsData = rows.filter((v,i) => i !== userIndex);
				...
				<EnhancedTableToolbar csvData={csvData} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} />
					...
					{permission.view ?   
						<div className="kt-section">
								<div>
									<table className="table table-striped- table-bordered table-hover table-checkable" id={"specialities"}>
											<thead>
											<tr>
