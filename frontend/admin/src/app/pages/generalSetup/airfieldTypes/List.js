...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('airfield/'+selected[0]+'/', data).then(function (response) {
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/airfield/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
function updateTable(newData, type) {
    const table = $('#airfield').DataTable();
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abairfieldtypes'),
    edit : userHasPermission('change_abairfieldtypes'),
    delete : userHasPermission('delete_abairfieldtypes'),
    view : userHasPermission('view_abairfieldtypes'),
  });
  let history = useHistory();
  let type = props.match.params.type;  

  React.useEffect(() => {
    if($.fn.dataTable.isDataTable( '#airfield' )) {
      let table = $('#airfield').DataTable();
      table.destroy();
      setTable(true);
    }
    return () => {
          setRowsData([]);
          let dtable = $('#airfield').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
}, []);
...
React.useEffect(() => {
  if (table && $.fn.dataTable.isDataTable( '#airfield' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'airfield?format=datatables';
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
          ReactDOM.render(
          <a onClick={() => history.push("/admin/airfield/"+rowData.id) }>
          {cellData}
          </a>
              , td
          );
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/airfield/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#airfield').DataTable({
      ...
      serverSide:true,
      displayStart : getPageNum('airfield', props), 
      ajax: {
        url:url ,
        method: "GET",
    ...
    $('#airfield').on( 'page.dt', function () {             
      setPageNum($('#airfield'), 'airfield', props);
    });
  }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('airfield/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('airfield/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
    ...
    <div className={classes.root}>
      ...
      <Paper className={classes.paper}>
        ...
        <div className="kt-portlet kt-portlet--mobile">
          <EnhancedTableToolbar csvData={csvData} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} type={type} />
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"airfield"}>
                      <thead>
                      <tr>
