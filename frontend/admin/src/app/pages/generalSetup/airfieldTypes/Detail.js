...
class Detail extends React.Component {
  ...
  constructor(props) {
    super(props);
    const { airfield_id, type } = this.props.data.match.params
    const divStyle = {
      padding : '15px'
    };
    ...
    this.getAirfield(airfield_id);
    this.divStyle = divStyle;
  }

  getAirfield(airfield_id) {
    list('airfield/'+airfield_id+'/').then(
      (response) => {
          ...
          this.setState({airfield : response.data})
    });
  }

  handleChange(val, attr) {
    var airfield = this.state.airfield;

    airfield[attr] = val;
    this.setState({airfield : airfield})
    patch('airfield/'+this.state.airfield.id+'/', this.state.airfield).then(
      (response) => {
        this.setState({airfield : response.data});
    }).catch(error => {
  ...
  confirm() {
    if(this.state.action === 'status') {
      this.state.airfield.is_active = this.state.is_active;
      patch('airfield/'+this.state.airfield.id+'/', this.state.airfield).then(
        (response) => {
          delete response.data.type;
          this.setState({airfield : response.data});
  ...
  render() {
    const { validated, airfield } = this.state;
    return (
        <div>
                  ...
                  <div className="kt_section__detail">
                    <div className="row mb-4">
                        <div className="col-md-6 col-12">
                            ...
                            <div>{airfield.name ? airfield.name : '---'}</div>
                        </div>
                        <div className="col-md-6 col-12">
                    ...
                    <div className="row mb-4">
                        ...
                        <div className="col-md-6 col-12">
                            ...
                            <div>
                              <span onClick={(e) => this.handleModalShow(e,'status')} className={this.state.airfield.is_active === 1 ? 'kt-switch kt-switch--sm kt-switch--success':'kt-switch kt-switch--sm kt-switch--danger'}>
                                <label>
                                  <input
                                    type="checkbox" checked={airfield.is_active === 1 ? 'defaultChecked':''}
                                    value={airfield.is_active === 1 ? '1' : '0'}
                                    name="is_active"
                                  />
                                  <span />
                                </label>
...
class DetailPage extends React.Component {
  constructor(props) {
    ...
    this.state = {errors:{}, showError:false, title: 'Airfield Types'};
            ...
            <CustomHead
              ...
              <div className="kt-portlet__head-wrapper">
                <div className="kt-portlet__head-actions">
                  ...
                  <Link to={"/admin/airfield"} className="btn btn-clean btn-icon-sm">
                        <i className="la la-long-arrow-left"></i>
                        Back
                      </Link>
                      <Link to={"/admin/airfield/"+this.props.match.params.airfield_id+"/edit"} className="btn btn-primary">
