...
class Detail extends React.Component {
  constructor(props) {
    super(props);
    const { department_id, type } = this.props.data.match.params;
    const divStyle = {
      padding: "15px",
    };
    ...
    list("departments/" + department_id + "/").then((response) => {
      ...
      this.setState({
        created_at: response.data.created_at
          ? new Intl.DateTimeFormat().format(new Date(response.data.created_at))
          : "",
        updated_at: response.data.updated_at
          ? new Intl.DateTimeFormat().format(new Date(response.data.updated_at))
          : "",
      });
  ...
  handleChange(val, attr) {
    ...
    departments[attr] = val;
    this.setState({ departments: departments });
    patch(
      "departments/" + this.state.departments.id + "/",
  ...
  confirm() {
    if (this.state.action === "status") {
      this.state.departments.is_active = this.state.is_active;
      patch(
        "departments/" + this.state.departments.id + "/",
        this.state.departments
      )
  ...
  render() {
    const { validated, departments } = this.state;
    return (
      <div>
            ...
            <div className="kt_section__detail">
              <div className="row mb-4">
                <div className="col-md-6 col-12">
                  ...
                  <div>{departments.name ? departments.name : "---"}</div>
                </div>
                <div className="col-md-6 col-12">
              ...
              <div className="row mb-4">
                <div className="col-md-6 col-12">
                  ...
                  <div>
                    {this.state.updated_at ? this.state.updated_at : "---"}
                  </div>
                </div>
                <div className="col-md-6 col-12">
                  ...
                  <div>
                    <span
                      onClick={(e) => this.handleModalShow(e, "status")}
                      className={
                        this.state.departments.is_active === 1
                    ...
                    >
                      <label>
                        <input
                          type="checkbox"
                          checked={
                            departments.is_active === 1 ? "defaultChecked" : ""
                          }
                          value={departments.is_active === 1 ? "1" : "0"}
                          name="is_active"
                        />
                        <span />
                      </label>
...
class DetailPage extends React.Component {
  constructor(props) {
    ...
    this.state = {errors:{}, showError:false, title: 'Department'};
