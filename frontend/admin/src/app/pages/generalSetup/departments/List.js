  ...
  const { exportSpinner,setExpSpin, csvData, setCsvData, numSelected, type, selected, setSelected, setSelectedMany, setRowsData, rows, setTemp } = props;
	...
	function handleDelAll(event) {
		if(selected.length) {
			...
			del("departments/" + selected[0] + "/", data).then(function(response) {
	...
	function fetchCSVData() {
		setExpSpin('');
		list("departments", { records: "all" }).then((response) => {
      setExpSpin("d-none");

      setCsvData(response.data);
      document.getElementById("departments-csv-btn").click();
    });
	}
	return (
		<Toolbar
								...
								<div className="kt-portlet__head-actions">
											{numSelected > 0 ? (
													...
													<div className="btn-group">
														<Link to={"/admin/departments/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
function updateTable(newData, type) {
		const table = $('#departments').DataTable();
...
export default function EnhancedTable(props) {
    ...
    create: userHasPermission("add_abdepartments"),
    edit: userHasPermission("change_abdepartments"),
    delete: userHasPermission("delete_abdepartments"),
    view: userHasPermission("view_abdepartments"),
  });
	...
	React.useEffect(() => {
		...
		return () => {
          setRowsData([]);
          let dtable = $("#departments").DataTable();
          dtable.destroy(true);
          setTable(true);
        }
}, []);
...
React.useEffect(() => {
	if (table && $.fn.dataTable.isDataTable("#departments")) {
	...
	function loadDatatable() {
		let url = API_URL + "departments?format=datatables";
		let columnDefs = [
			...
			},
			{
				...
				"createdCell": function (td, cellData, rowData, row, col) {
					ReactDOM.render(
            <a onClick={() => history.push("/admin/departments/" + rowData.id)}>
              {cellData}
            </a>,
            td
          );
                  ...
                  <Dropdown.Menu className="dropdown-menu-fit dropdown-menu-anim dropdown-menu-top-unround">
                    {permission.edit ? (
                      <Dropdown.Item
                        onClick={() =>
                          history.push(
                            "/admin/departments/" + rowData.id + "/edit"
                          )
                        }
                      >
      ...
      serverSide: true,
      displayStart: getPageNum("departments", props),
      ajax: {
        url: url,
        method: "GET",
        headers: {
          ...
          Authorization:
            "Token " +
            JSON.parse(
              localStorage.getItem("persist:demo1-auth")
            ).authToken.replace(/['"]+/g, ""),
        },
      ...
      columnDefs: columnDefs,
		...
		$("#departments").on("page.dt", function() {
      setPageNum($("#departments"), "departments", props);
    }); 
	}
	...
	function confirm(event) {
		if(action == 'status') {
			...
			patch("departments/" + rows[userIndex].id + "/", data).then(function(
        response
      ) {
        ...
        setOpen(true);
        const statusType =
          response.data.is_active === 1 ? "activated" : "deactivated";
        setMessage("Record successfully " + statusType);
      });
		} else if(action == 'delete') {
			del("departments/" + rows[userIndex].id + "/").then(function(response) {
        let rowsData = rows.filter((v, i) => i !== userIndex);
