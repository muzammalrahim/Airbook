  ...
  const { exportSpinner,setExpSpin, csvData, setCsvData, numSelected, type, selected, setSelected, setSelectedMany, setRowsData, rows, setTemp } = props;
	...
	function handleDelAll(event) {
		if(selected.length) {
			...
			del('job_titles/'+selected[0]+'/', data).then(function (response) {
	...
	function fetchCSVData() {
		setExpSpin('');
		list('job_titles', {records:'all'}).then(response => {
			setExpSpin('d-none');

		setCsvData(response.data);
		document.getElementById('job_titles-csv-btn').click();
	})
	}
	return (
		<Toolbar
								...
								<div className="kt-portlet__head-actions">
											{numSelected > 0 ? (
													...
													<div className="btn-group">
														<Link to={"/admin/titles/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
export default function EnhancedTable(props) {
	...
	const [permission, setPermission] = React.useState({
		create : userHasPermission('add_abtitles'),
		edit : userHasPermission('change_abtitles'),
		delete : userHasPermission('delete_abtitles'),
		view : userHasPermission('view_abtitles'),
	});
	let history = useHistory();
	
	React.useEffect(() => {
		if($.fn.dataTable.isDataTable( '#titles' )) {
			let table = $('#titles').DataTable();
			table.destroy();
			setTable(true);
		}
		return () => {
          setRowsData([]);
          let dtable = $('#titles').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
}, []);
...
React.useEffect(() => {
	if (table && $.fn.dataTable.isDataTable( '#titles' ) ) {
	...
	function loadDatatable() {
		let url = API_URL+'job_titles?format=datatables';
		let columnDefs = [
			...
			},
			{
				...
				"createdCell": function (td, cellData, rowData, row, col) {
					ReactDOM.render(
					<a onClick={() => history.push("/admin/titles/"+rowData.id) }>
					{cellData}
					</a>
							, td
					);
			...
			columnDefs.push(
					{
						...
						"createdCell": function (td, cellData, rowData, row, col) {
							...
							{permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/titles/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
		...
		$('#titles').DataTable({
			...
			serverSide:true,
			displayStart : getPageNum('job_titles', props),
			ajax: {
				url:url ,
				method: "GET",
		...
		$('#titles').on( 'page.dt', function () {             
			setPageNum($('#titles'), 'job_titles', props);
		}); 
	}
	...
	function confirm(event) {
		if(action == 'status') {
			...
			patch('job_titles/'+rows[userIndex].id+'/', data).then(function (response) {
			...
			del('job_titles/'+rows[userIndex].id+'/').then(function (response) {
				let rowsData = rows.filter((v,i) => i !== userIndex);
				...
				<EnhancedTableToolbar  exportSpinner={exportSpinner} setExpSpin={setExpSpin} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} csvData={csvData} setCsvData={setCsvData} />
					...
					{permission.view ?   
						<div className="kt-section">
								<div>
									<table className="table table-striped- table-bordered table-hover table-checkable" id={"titles"}>
											<thead>
											<tr>
