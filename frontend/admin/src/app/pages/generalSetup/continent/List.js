...
const EnhancedTableToolbar = props => {
	...
	function handleDelAll(event) {
		if(selected.length) {
			...
			del('continent/'+selected[0]+'/', data).then(function (response) {
								...
								<div className="kt-portlet__head-actions">
											{numSelected > 0 ? (
													...
													<div className="btn-group">
														<Link to={"/admin/continent/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
function updateTable(newData, type) {
		const table = $('#continent').DataTable();
...
export default function EnhancedTable(props) {
	...
	const [permission, setPermission] = React.useState({
		create : userHasPermission('add_abcontinents'),
		edit : userHasPermission('change_abcontinents'),
		delete : userHasPermission('delete_abcontinents'),
		view : userHasPermission('view_abcontinents'),
	});
	let history = useHistory();
	
	React.useEffect(() => {
		if($.fn.dataTable.isDataTable( '#continent' )) {
			let table = $('#continent').DataTable();
			table.destroy();
			setTable(true);
		}
		return () => {
          setRowsData([]);
          let dtable = $('#continent').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
	}, []);
...
React.useEffect(() => {
	if (table && $.fn.dataTable.isDataTable( '#continent' ) ) {
	...
	function loadDatatable() {
		let url = API_URL+'continent?format=datatables';
		let columnDefs = [
			...
			},
			{
				...
				"createdCell": function (td, cellData, rowData, row, col) {
					ReactDOM.render(
					<a onClick={() => history.push("/admin/continent/"+rowData.id) }>
					{cellData}
					</a>
							, td
					);
			...
			columnDefs.push(
					{
						...
						"createdCell": function (td, cellData, rowData, row, col) {
							...
							{permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/continent/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
		...
		$('#continent').DataTable({
			...
			serverSide:true,
			displayStart : getPageNum('continent', props),
			ajax: {
				url:url ,
				method: "GET",
		...
		$('#continent').on( 'page.dt', function () {             
			setPageNum($('#continent'), 'continent', props);
		});
	}
	...
	function confirm(event) {
		if(action == 'status') {
			...
			patch('continent/'+rows[userIndex].id+'/', data).then(function (response) {
			...
			del('continent/'+rows[userIndex].id+'/').then(function (response) {
				let rowsData = rows.filter((v,i) => i !== userIndex);
				...
				<EnhancedTableToolbar csvData={csvData} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} />
					...
					{permission.view ?   
						<div className="kt-section">
								<div>
									<table className="table table-striped- table-bordered table-hover table-checkable" id={"continent"}>
											<thead>
											<tr>
