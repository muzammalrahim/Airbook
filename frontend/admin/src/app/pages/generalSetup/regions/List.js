...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('region/'+selected[0]+'/', data).then(function (response) {
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/region/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abregions'),
    edit : userHasPermission('change_abregions'),
    delete : userHasPermission('delete_abregions'),
    view : userHasPermission('view_abregions'),
  });
  let history = useHistory();
  React.useEffect(() => {
		if($.fn.dataTable.isDataTable( '#regions' )) {
          let table = $('#regions').DataTable();
          table.destroy();
          setTable(true);
		}
		return () => {
          setRowsData([]);
          let dtable = $('#regions').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
  }, []);
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#regions' ) ) {
    ...
    function loadDatatable() {
      let url = API_URL+'region?format=datatables';
      let columnDefs = [
        ...
        },
        {
          ...
          "createdCell": function (td, cellData, rowData, row, col) {
            ReactDOM.render(
            <a onClick={() => history.push("/admin/region/"+rowData.id) }>
            {cellData}
            </a>
                , td
            );
        ...
        columnDefs.push(
            {
              ...
              "createdCell": function (td, cellData, rowData, row, col) {
                ...
                {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/region/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
      ...
      $('#regions').DataTable({
        ...
        serverSide:true,
        displayStart : getPageNum('region', props),
        ajax: {
          url:url ,
          method: "GET",
      ...
      $('#regions').on( 'page.dt', function () {             
        setPageNum($('#regions'), 'region', props);
    });
    }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('region/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('region/'+rows[userIndex].id+'/').then(function (response) {
				let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <EnhancedTableToolbar csvData={csvData} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} />
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id="regions">
                      <thead>
                      <tr>
