...
import { list, patch, del, API_URL, userHasPermission, headers, importData} from "../../crud/api";
...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('events/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('events', {records:'all'}).then(response => {
      let related_models = ['continent','region','country','state','city','location']
      ...
      let responseData = response.data.map((v, i) => {
        ...
        v.categories = v.categories ? (v.categories.map((v)=>{return v.name })).join() : null
        return v;
      })
      setCsvData(responseData);
      document.getElementById('event-csv-btn').click();
    })
  }
        ...
        <div className="kt-portlet__head kt-portlet__head--lg">
            <div className="kt-portlet__head-label">
              <span className="kt-portlet__head-icon">
                <i className="kt-font-brand flaticon-event-calendar-symbol" />
              </span>
                {numSelected > 0 ? (
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/events/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
                                  ...
                                  <ul className="kt-nav">
                                    ...
                                    <li className="kt-nav__item">
                                      <a className="kt-nav__link">
                                        <i className="kt-nav__link-icon la la-upload" />
                                        <CSVReader inputId="events-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbEvents').then((response)=>{
...
function parseRowData(rowData) {
    ...
    rowData.forEach(function(item, index){
      ...
      rowData[index]['categories__name'] = categories;
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abevents'),
    edit : userHasPermission('change_abevents'),
    delete : userHasPermission('delete_abevents'),
    view : userHasPermission('view_abevents'),
  });
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#events' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'events?format=datatables';
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
          ReactDOM.render(
          <a onClick={() => history.push("/admin/events/"+rowData.id) }>
          {cellData}
          </a>
              , td
          );
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/events/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#events').DataTable({
      ...
      serverSide:true,
      displayStart : getPageNum('events', props),
      ajax: {
        url:url ,
        method: "GET",
    ...
    $('#events').on( 'page.dt', function () {             
      setPageNum($('#events'), 'events', props);
    });
  }
  ...
  function handleSelectAllClick(event) {
    ...
    if (event.target.checked) {
        const sel = selected;
      const eventsSelected = rows.map(n => n.id);
        eventsSelected.forEach(v => {
         sel.push(v)
       })
      setSelected(sel);
    }else{
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('events/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('events/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"events"}>
                      <thead>
                      <tr>
                          ...
                          <th data-data="title">Name</th>
                          <th data-data="categories__name">Category</th>
