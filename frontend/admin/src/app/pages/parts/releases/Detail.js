...
class Detail extends React.Component {
  ...
  constructor(props) {
    super(props);
    const { release_id, type } = this.props.data.match.params
    const divStyle = {
      padding : '15px'
    };
  ...
  handleChange(val, attr) {
    ...
    patch('releases/'+this.state.releases.id+'/', this.state.releases).then(
      (response) => {
        this.setState({releases : response.data});
    }).catch(error => {
  ...
  confirm() {
    if(this.state.action === 'status') {
      this.state.releases.is_active = this.state.is_active;
      patch('releases/'+this.state.releases.id+'/', this.state.releases).then(
        (response) => {
          delete response.data.type;
          this.setState({releases : response.data});
  ...
  render() {
    const { validated, releases } = this.state;
    return (
        <div>
                  ...
                  <div className="kt_section__detail">
                    <div className="row mb-4">
                        <div className="col-md-6 col-12">
                            ...
                            <div>{releases.name ? releases.name : '---'}</div>
                        </div>
                        <div className="col-md-6 col-12">
                    ...
                    <div className="row mb-4">
                        ...
                        <div className="col-md-6 col-12">
                            ...
                            <div>
                              <span onClick={(e) => this.handleModalShow(e,'status')} className={this.state.releases.is_active === 1 ? 'kt-switch kt-switch--sm kt-switch--success':'kt-switch kt-switch--sm kt-switch--danger'}>
                                <label>
                                  <input
                                    type="checkbox" checked={releases.is_active === 1 ? 'defaultChecked':''}
                                    value={releases.is_active === 1 ? '1' : '0'}
                                    name="is_active"
                                  />
                                  <span />
                                </label>
...
class DetailPage extends React.Component {
  constructor(props) {
    ...
    this.state = {errors:{}, showError:false, title: 'Release'};
              ...
              <div className="kt-portlet__head-wrapper">
                <div className="kt-portlet__head-actions">
                  ...
                  <Link to={"/admin/parts/release"} className="btn btn-clean btn-icon-sm">
                      ...
                      <Link to={"/admin/parts/release/"+this.props.match.params.release_id+"/edit"} className="btn btn-primary">
