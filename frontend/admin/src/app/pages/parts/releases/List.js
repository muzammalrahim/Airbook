...
import { list, patch, del, userHasPermission, importData, API_URL} from "../../../crud/api";
...
const EnhancedTableToolbar = props => {
  ...
  const {  setMessageType, setOpen, setMessage, exportSpinner,setExpSpin, csvData, setCsvData, numSelected, selected, setSelected, setSelectedMany, setRowsData, rows, setTemp} = props;


  function handleDelAll(event) {
    if(selected.length) {
      let data = {ids:selected}
      del('releases/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('releases', {records:'all'}).then(response => {
      setExpSpin('d-none');
      setCsvData(response.data);
      document.getElementById('releases-csv-btn').click();
    })
  }
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/parts/release/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
                                  ...
                                  <ul className="kt-nav">
                                    ...
                                    <li className="kt-nav__item">
                                      <a className="kt-nav__link">
                                        <i className="kt-nav__link-icon la la-upload" />
                                        <CSVReader inputId="releases-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" 
                                        onFileLoaded={data => importData(data, 'AbReleases')
                                        .then((response)=>{
...
function updateTable(newData, type) {
    const table = $('#parts_release').DataTable();
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abreleases'),
    edit : userHasPermission('change_abreleases'),
    delete : userHasPermission('delete_abreleases'),
    view : userHasPermission('view_abreleases'),
  });
  let history = useHistory();
  React.useEffect(() => {
    if($.fn.dataTable.isDataTable( '#parts_release' )) {
      let table = $('#parts_release').DataTable();
      table.destroy();
      setTable(true);
    }
    return () => {
          setRowsData([]);
          let dtable = $('#parts_release').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
  }, []);
  ...
  React.useEffect(() => {
    if ( table && $.fn.dataTable.isDataTable( '#parts_release' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'releases?format=datatables';
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
          ReactDOM.render(
          <a onClick={() => history.push("/admin/parts/release/"+rowData.id) }>
          {cellData}
          </a>
              , td
          );
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/parts/release/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#parts_release').DataTable({
      ...
      serverSide:true,
      displayStart : getPageNum('releases', props), 
      ajax: {
        url:url ,
        method: "GET",
    ...
    $('#parts_release').on( 'page.dt', function () {             
      setPageNum($('#parts_release'), 'releases', props);
    });  
  }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('releases/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('releases/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
  ...
  return (
    ...
    <div className={classes.root}>
      ...
      <Paper className={classes.paper}>
        ...
        <EnhancedTableToolbar  setMessageType={setMessageType} setOpen={setOpen} setMessage={setMessage} exportSpinner={exportSpinner} setExpSpin={setExpSpin} csvData={csvData} setCsvData={setCsvData} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length}/>
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"parts_release"}>
                      <thead>
                      <tr>
