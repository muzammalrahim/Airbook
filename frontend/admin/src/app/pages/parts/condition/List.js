...
import { list, patch, del, userHasPermission, importData, API_URL, headers} from "../../../crud/api";
...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('conditions/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('conditions', {records:'all'}).then(response => {
      setExpSpin('d-none');
      setCsvData(response.data);
      document.getElementById('conditions-csv-btn').click();
    })
  }
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/parts/condition/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
                                  ...
                                  <ul className="kt-nav">
                                    ...
                                    <li className="kt-nav__item">
                                      <a className="kt-nav__link">
                                        <i className="kt-nav__link-icon la la-upload" />
                                        <CSVReader inputId="conditions-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbConditions').then((response)=>{
...
function updateTable(newData, type) {
    const table = $('#parts_condition').DataTable();
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abconditions'),
    edit : userHasPermission('change_abconditions'),
    delete : userHasPermission('delete_abconditions'),
    view : userHasPermission('view_abconditions'),
  });
  let history = useHistory();

  React.useEffect(() => {
    if($.fn.dataTable.isDataTable( '#parts_condition' )) {
      let table = $('#parts_condition').DataTable();
      table.destroy();
      setTable(true);
    }
    return () => {
          setRowsData([]);
          let dtable = $('#parts_condition').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
  }, []);
  ...
  React.useEffect(() => {
    if ( table && $.fn.dataTable.isDataTable( '#parts_condition' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'conditions?format=datatables';
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
          ReactDOM.render(
          <a onClick={() => history.push("/admin/parts/condition/"+rowData.id) }>
          {cellData}
          </a>
              , td
          );
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/parts/condition/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#parts_condition').DataTable({
      ...
      serverSide:true,
      displayStart : getPageNum('conditions', props),
      ajax: {
        url:url ,
        method: "GET",
    ...
    $('#parts_condition').on( 'page.dt', function () {             
      setPageNum($('#parts_condition'), 'conditions', props);
    }); 
  }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('conditions/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('conditions/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"parts_condition"}>
                      <thead>
                      <tr>
