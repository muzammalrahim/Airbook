...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('seos/'+selected[0]+'/', data).then(function (response) {
        ...
        <div className="kt-portlet__head kt-portlet__head--lg">
            <div className="kt-portlet__head-label">
              <span className="kt-portlet__head-icon">
                <i className="kt-font-brand flaticon-cogwheel" />
              </span>
                {numSelected > 0 ? (
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/seos/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
                                ...
                                <ul className="kt-nav">
                                  ...
                                  <li className="kt-nav__item">
                                    <a className="kt-nav__link">
                                      ...
                                      <CSVReader inputId="seo-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbSeos')} />
                                    </a>
                                  </li>
                                </ul>
                              </Dropdown.Menu>
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abseos'),
    edit : userHasPermission('change_abseos'),
    delete : userHasPermission('delete_abseos'),
    view : userHasPermission('view_abseos'),
  });
  let history = useHistory();

  React.useEffect(() => {
		if($.fn.dataTable.isDataTable( '#seos' )) {
		let table = $('#seos').DataTable();
		table.destroy();
		setTable(true);
		}
  }, []);
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#seos' ) ) {
    ...
    function loadDatatable() {
      let url = API_URL+'seos?format=datatables';
      let columnDefs = [
        ...
        },
        {
          ...
          "createdCell": function (td, cellData, rowData, row, col) {
            ReactDOM.render(
            <a onClick={() => history.push("/admin/seos/"+rowData.id) }>
            {cellData}
            </a>
                , td
            );
        ...
        columnDefs.push(
            {
              ...
              "createdCell": function (td, cellData, rowData, row, col) {
                ...
                {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/seos/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
      ...
      $('#seos').DataTable({
        ...
        serverSide:true,
        displayStart : getPageNum('seos', props),
        ajax: {
          url:url ,
          method: "GET",
      ...
      $('#seos').on( 'page.dt', function () {
        setPageNum($('#seos'), 'seos', props);
    });
    }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('seos/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('seos/'+rows[userIndex].id+'/').then(function (response) {
				let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <EnhancedTableToolbar csvData={csvData} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} />
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"seos"}>
                      <thead>
                      <tr>
                          ...
                          <th data-data="model_type">Model Type</th>
