...
import {post, SECTIONS} from "../../../crud/api";
...
class Create extends React.Component {
    ...
    handleSubmit(event) {
        ...
        post('cms', this.state.cms).then(
            (response) => {
                this.setState({cms: response.data});
                this.state.action == 'save_new' ? this.clearForm("static-page-from") : this.props.data.history.push("/admin/cms");
            }).catch(error => {
    ...
    handleFileRemove = (event) => {
        let cms = this.state.cms;
        cms.file = {}
        this.setState({selectedFile: null, cms: cms})
        document.getElementById('cms-image-upload').value = '';
    }
    clearForm = (id) => {
        this.props.data.history.replace("/admin/cms");
        this.props.data.history.replace("/admin/cms/create");
    }
                ...
                <Form.Row>
                    <Form.Group as={Col} md="4">
                        ...
                        <Select
                            ...
                            styles={customStyles}
                            onChange={e => this.selectCMS(e, 'section')}
                            options={SECTIONS}
                        />
                    </Form.Group>
                    <Form.Group as={Col} md="4">
