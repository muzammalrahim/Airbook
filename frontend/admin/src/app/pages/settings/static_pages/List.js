...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('cms/'+selected[0]+'/', data).then(function (response) {
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/cms/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
                                ...
                                <ul className="kt-nav">
                                  ...
                                  <li className="kt-nav__item">
                                    <a className="kt-nav__link">
                                      ...
                                      <CSVReader inputId="cms-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbCms')} />
                                    </a>
                                  </li>
                                </ul>
                              </Dropdown.Menu>
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abcms'),
    edit : userHasPermission('change_abcms'),
    delete : userHasPermission('delete_abcms'),
    view : userHasPermission('view_abcms'),
  });
  let history = useHistory();

  React.useEffect(() => {
		if($.fn.dataTable.isDataTable( '#cms' )) {
		let table = $('#cms').DataTable();
		table.destroy();
		setTable(true);
		}
  }, []);
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#cms' ) ) {
    ...
    function loadDatatable() {
      let url = API_URL+'cms?format=datatables';
      let columnDefs = [
        ...
        },
        {
          ...
          "createdCell": function (td, cellData, rowData, row, col) {
            ReactDOM.render(
            <a onClick={() => history.push("/admin/cms/"+rowData.id) }>
            {cellData}
            </a>
                , td
            );
        ...
        columnDefs.push(
            {
              ...
              "createdCell": function (td, cellData, rowData, row, col) {
                ...
                {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/cms/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
      ...
      $('#cms').DataTable({
        ...
        serverSide:true,
        displayStart : getPageNum('cms', props),
        ajax: {
          url:url ,
          method: "GET",
      ...
      $('#cms').on( 'page.dt', function () {
        setPageNum($('#cms'), 'cms', props);
    });
    }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('cms/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('cms/'+rows[userIndex].id+'/').then(function (response) {
				let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <EnhancedTableToolbar csvData={csvData} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} />
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"cms"}>
                      <thead>
                      <tr>
                          ...
                          <th data-data="sub_title">Sub Title</th>
