...
const EnhancedTableToolbar = props => {
  ...
  const { setMessageType, setMessage, setOpen, exportSpinner,setExpSpin, numSelected, type, selected, setSelected, setSelectedMany, setRowsData, rows, setTemp } = props;
  function handleDelAll(event) {
    if(selected.length) {
      let data = {ids:selected}
      del('pricing/'+selected[0]+'/', data).then(function (response) {
        ...
        <div className="kt-portlet__head kt-portlet__head--lg">
            <div className="kt-portlet__head-label">
              <span className="kt-portlet__head-icon">
                { type && (type === 'event' || type === 'news')  ? (
                    <i className="kt-font-brand flaticon2-open-box" />
                ) : (
                    <i className="kt-font-brand flaticon-price-tag" />
                    )
                }
              </span>
                {numSelected > 0 ? (
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/pricing/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abpricing'),
    edit : userHasPermission('change_abpricing'),
    delete : userHasPermission('delete_abpricing'),
    view : userHasPermission('view_abpricing'),
  });
  ...
  React.useEffect(() => {
   ...
   console.log('property changed', props.location);
  },[props.location])


  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#pricing' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'pricing?format=datatables';
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
          ReactDOM.render(
              <a style={{ cursor: 'pointer' }} onClick={() => history.push("/admin/pricing/"+rowData.id) }>
                  {cellData}
                </a>
              , td
          );
                    ...
                    <Dropdown className="kt-header__topbar-item" drop="down" alignLeft>
                        <Dropdown.Toggle as={ActionsDropdownToggle} />
                            <Dropdown.Menu  className="dropdown-menu-fit dropdown-menu-anim dropdown-menu-top-unround">
                              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/pricing/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#pricing').DataTable({
      ...
      displayStart : getPageNum('pricing', props),
      ajax: {
        url:url ,
        method: "GET",
    ...
    $('#pricing').on( 'page.dt', function () {
        setPageNum($('#pricing'), 'pricing', props);
    });
  }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('pricing/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('pricing/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
    ...
    <div className={classes.root}>
      ...
      <Paper className={classes.paper}>
        ...
        <div className="kt-portlet kt-portlet--mobile">
            <EnhancedTableToolbar setMessageType={setMessageType} setMessage={setMessage} setOpen={setOpen} exportSpinner={exportSpinner} setExpSpin={setExpSpin} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} />
          ...
          {permission.view ?
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"pricing"}>
                      <thead>
                      <tr>
