...
import { list, patch, del, userHasPermission, importData, headers, API_URL} from "../../../crud/api";
...
const EnhancedTableToolbar = props => {
  ...
  const {setExpSpin, exportSpinner, setCsvData,  setMessageType, setMessage, setOpen,  numSelected, type, selected, setSelected, setSelectedMany, setRowsData, rows, setTemp, csvData } = props;


  function handleDelAll(event) {
    if(selected.length) {
      let data = {ids:selected}
      del('subscribers/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('subscribers', {records:'all'}).then(response => {
      let related_models = []
      ...
      document.getElementById('subscriber-csv-btn').click();
    })
  }
                ...
                <div className="kt-portlet__head-actions">
                  <div className="dropdown dropdown-inline">
                      {numSelected > 0 ? (
                        ...
                        <>
                            <Link to={"/admin/subscriber/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm">
                              ...
                              <ul className="kt-nav">
                                ...
                                <li className="kt-nav__item">
                                  <a className="kt-nav__link">
                                    <i className="kt-nav__link-icon la la-upload" />
                                    <CSVReader inputId="subscriber-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbSubscribers').then((response)=>{
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_absubscribers'),
    edit : userHasPermission('change_absubscribers'),
    delete : userHasPermission('delete_absubscribers'),
    view : userHasPermission('view_absubscribers'),
  });
  let history = useHistory();
  React.useEffect(() => {
		if($.fn.dataTable.isDataTable( '#subscribers' )) {
		...
		return () => {
          setRowsData([]);
          let dtable = $('#subscribers').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
  }, []);
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#subscribers' ) ) {
    ...
    function loadDatatable() {
      let url = API_URL+'subscribers?format=datatables';
      let columnDefs = [
        ...
        },
        {
          ...
          "createdCell": function (td, cellData, rowData, row, col) {
            ReactDOM.render(
            <a onClick={() => history.push("/admin/subscriber/"+rowData.id) }>
            {cellData}
            </a>
                , td
            );
        ...
        columnDefs.push(
            {
              ...
              "createdCell": function (td, cellData, rowData, row, col) {
                ...
                {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/subscriber/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
      ...
      $('#subscribers').DataTable({
        ...
        serverSide:true,
        displayStart : getPageNum('subscribers', props), 
        ajax: {
          url:url ,
          method: "GET",
      ...
      $('#subscribers').on( 'page.dt', function () {             
        setPageNum($('#subscribers'), 'subscribers', props);
      });
    }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('subscribers/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('subscribers/'+rows[userIndex].id+'/').then(function (response) {
				let rowsData = rows.filter((v,i) => i !== userIndex);
  ...
  return (
    ...
    <div className={classes.root}>
      ...
      <Paper className={classes.paper}>
        ...
        <EnhancedTableToolbar setCsvData={setCsvData} setMessageType={setMessageType} setMessage={setMessage} setOpen={setOpen} csvData={csvData} exportSpinner={exportSpinner} setExpSpin={setExpSpin} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} />
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"subscribers"}>
                      <thead>
                      <tr>
