...
import { list, patch, del, userHasPermission, ENQUERIES_STATUS, API_URL} from "../../../crud/api";
...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('contactqueries/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('contactqueries', {records:'all'}).then(response => {
      ...
      document.getElementById('contactqueries-csv-btn').click();
    })
  }
                                ...
                                <Dropdown.Menu className="dropdown-menu-fit dropdown-menu-md dropdown-menu-right">
                                  <ul className="kt-nav">
                                    <li className="kt-nav__item">
                                      ...
                                      <CSVLink
                                        data={csvData}
                                        filename="contactqueries.csv"
                                        className="d-none"
                                        id="contactqueries-csv-btn"
                                        target="_blank" 
                                     />
                                    </li>
                                    {/*<li className="kt-nav__item">
...
function updateTable(newData, type) {
    const table = $('#contactqueries').DataTable();
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    // create : userHasPermission('add_abcontactqueries'),
    edit : userHasPermission('change_abcontactqueries'),
    delete : userHasPermission('delete_abcontactqueries'),
    view : userHasPermission('view_abcontactqueries'),
  });
  let history = useHistory();
  React.useEffect(() => {
		if($.fn.dataTable.isDataTable( '#contactqueries' )) {
		...
		return () => {
          setRowsData([]);
          let dtable = $('#contactqueries').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
  }, []);
  ...
  function changeStatus(event,index) {
    ...
    patch('contactqueries/'+rowsData[index].id+'/', data).then(function (response) {
      let rows = rowsData.filter((v,i) => i !== index);
        setRowsData([...rows, response.data]);
        setMessage('Status successfully updated');
    })
  }
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#contactqueries' ) ) {
    ...
    function loadDatatable() {
      let url = API_URL+'contactqueries?format=datatables';
      let columnDefs = [
        ...
        },
        {
          ...
          "createdCell": function (td, cellData, rowData, row, col) {
            ReactDOM.render(
            <a onClick={() => history.push("/admin/contactqueries/"+rowData.id) }>
            {cellData}
            </a>
                , td
            );
        ...
        },
        {
          ...
          "createdCell": function (td, cellData, rowData, row, col) {
            let defaultValue = '';
            ReactDOM.render(
              <Form.Control onChange={(e) => changeStatus(e, row)} as="select">
                {
                ENQUERIES_STATUS.map((val, i) => { 
                  if(val.value === cellData)
                    defaultValue = 'selected'
                  else
                    defaultValue = '';
                 return <option selected={defaultValue} key={i} value={val.value}>{val.label}</option> 
               })}
              </Form.Control>
                , td
            );
      ...
      $('#contactqueries').DataTable({
        ...
        serverSide:true,
        displayStart : getPageNum('contactqueries', props),
        ajax: {
          url:url ,
          method: "GET",
      ...
      $('#contactqueries').on( 'page.dt', function () {             
        setPageNum($('#contactqueries'), 'contactqueries', props);
      }); 
    }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('contactqueries/'+rows[userIndex].id+'/', data).then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
        console.log('reo', rowsData)
      ...
      del('contactqueries/'+rows[userIndex].id+'/').then(function (response) {
				let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"contactqueries"}>
                      <thead>
                        <tr>
                            ...
                            <th data-data="enquiry_type">Enquiry Type</th>
