...
const EnhancedTableToolbar = props => {
  ...
  const { setMessageType, setMessage, setOpen, exportSpinner,setExpSpin, csvData, setCsvData, numSelected, selected, setSelected, setSelectedMany, setRowsData, rows, setTemp } = props;

  function handleDelAll(event) {
    if(selected.length) {
      let data = {ids:selected}
      del('airport/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('airports', {records:'all'}).then(response => {
      let related_models = ['city','state', 'country','user','airfield_type']
      ...
      document.getElementById('airport-csv-btn').click();
    })
  }
        ...
        <div className="kt-portlet__head kt-portlet__head--lg">
            <div className="kt-portlet__head-label">
              <span className="kt-portlet__head-icon">
                <i className="kt-font-brand fa fa-plane-arrival" />
              </span>
                {numSelected > 0 ? (
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/airport/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
                                  ...
                                  <ul className="kt-nav">
                                    ...
                                    <li className="kt-nav__item">
                                      <a className="kt-nav__link">
                                        <i className="kt-nav__link-icon la la-upload" />
                                        <CSVReader inputId="airport-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbAirports').then((response)=>{
...
function updateTable() {
    const table = $('#airports').DataTable();
...
function parseRowData(rowData) {
    ...
    rowData.forEach(function(item, index){
        if(item.city)
            rowData[index]['city__name'] = item.city.name;
        else
            rowData[index]['city__name'] = '';

        if(item.country)
            rowData[index]['country__name'] = item.country.name;
        else
            rowData[index]['country__name'] = '';
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abairports'),
    edit : userHasPermission('change_abairports'),
    delete : userHasPermission('delete_abairports'),
    view : userHasPermission('view_abairports'),
  });
  let history = useHistory();


  React.useEffect(() => {
      if($.fn.dataTable.isDataTable( '#airports' )) {
        let table = $('#airports').DataTable();
        table.destroy();
        setTable(true);
      }
      return () => {
          setRowsData([]);
          let dtable = $('#airports').DataTable();
          try {
            dtable.destroy(true);
          } catch(e) {
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#airports' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'airports?format=datatables';
    let columnDefs = [
      ...
      },
      {
          targets: 2,
          "createdCell": function (td, cellData, rowData, row, col) {
              const aiport_slug = rowData.id+'-'+rowData.name.replace(/\s+/g, '-').toLowerCase()
              ReactDOM.render(
                  <a target="_blank" style={{ cursor: 'pointer' }} href={"/airport/" + aiport_slug + "/"}>
                      {cellData}
                  </a>
                  , td
              );
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/airport/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#airports').DataTable({
      ...
      serverSide:true,
      displayStart : getPageNum('airports', props),
      ajax: {
        url:url ,
        method: "GET",
    ...
    $('#airports').on( 'page.dt', function () {             
      setPageNum($('#airports'), 'airports', props);
    }); 
  }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('airport/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('airport/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
    ...
    <div className={classes.root}>
      ...
      <Paper className={classes.paper}>
        ...
        <div className="kt-portlet kt-portlet--mobile">
            <EnhancedTableToolbar setMessageType={setMessageType} setMessage={setMessage} setOpen={setOpen} exportSpinner={exportSpinner} setExpSpin={setExpSpin} setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} selected={selected} numSelected={selected.length} csvData={csvData} setCsvData={setCsvData} />
          ...
          {permission.view ?
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"airports"}>
                      <thead>
                      <tr>
                          ...
                          <th data-data="city__name">City</th>
                          <th data-data="country__name">Country</th>
                          <th data-data="iata_code">IATA</th>
                          <th data-data="icao_code">ICAO</th>
