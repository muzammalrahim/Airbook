...
class Create extends React.Component {
  ...
  handleSubmit(event) {
    ...
    post('category', this.state.category).then(
      (response) => {
        ...
        this.state.action == 'save_new' ? this.clearForm(this.state.type+"-category-form") : this.props.data.history.push("/admin/"+this.state.type+"/category");
    }).catch(error => { 
  ...
  clearForm = (id) => {
    this.props.data.history.replace("/admin/"+this.state.type+"/category");
    this.props.data.history.replace("/admin/"+this.state.type+"/category/create");
  }
