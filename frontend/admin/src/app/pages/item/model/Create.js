...
class Create extends React.Component {
  ...
  handleSubmit(event) {
    ...
    post('model', this.state.model).then(
      (response) => {
        ...
        this.state.action == 'save_new' ? this.clearForm(this.state.type+"-model-form") : this.props.data.history.push("/admin/"+this.state.type+"/model");
      }).catch(error => { 
  ...
  clearForm = (id) => {
    this.props.data.history.replace("/admin/"+this.state.type+"/model");
    this.props.data.history.replace("/admin/"+this.state.type+"/model/create");
  }

  render() {
    const { validated, model, type_0, types, selectedFile, initialOptions, modelsLoaded } = this.state;
    return (
      <Form
        noValidate
        ...
        <Form.Row>
          ...
          <Form.Label>Type *</Form.Label>
            <AsyncPaginate
                ...
                value={type_0.value ? type_0 :'select...'}
                model="type"
                name="type"
                isClearable = {true}
                escapeClearsValue = {true}
                onChange={e => this.selectType(e, 'type_0')}
                loadOptions={(search, prevOptions) => loadOptions(search, prevOptions, types, modelsLoaded)}
            />
          </Form.Group>
