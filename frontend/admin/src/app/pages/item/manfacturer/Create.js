...
import { list, loadOptions, DROPDOWN_WAIT, post } from "../../../crud/api";
...
class Create extends React.Component {
	...
	handleSubmit(event) {
		...
		post('manufacturer', this.state.manufacturer).then(
			(response) => {
				...
				this.state.action == 'save_new' ? this.clearForm(this.state.type+"-manufacturer-form") : this.props.data.history.push("/admin/"+this.state.type+"/manufacturer");
			}).catch(error => { 
	...
	clearForm = (id) => {
		this.props.data.history.replace("/admin/"+this.state.type+"/manufacturer");
        this.props.data.history.replace("/admin/"+this.state.type+"/manufacturer/create");
	}
	render() {
		const { selectedFile, manufacturer, country, countries, category, initialOptions, modelsLoaded, categories } = this.state;
