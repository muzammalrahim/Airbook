...
const EnhancedTableToolbar = props => {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('contacts', {records:'all'}).then(response => {
      ...
      document.getElementById('contact-csv-btn').click();
    })
  }
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/contacts/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abcontacts'),
    edit : userHasPermission('change_abcontacts'),
    delete : userHasPermission('delete_abcontacts'),
    view : userHasPermission('view_abcontacts'),
  });
  ...
  React.useEffect(() => {
      if($.fn.dataTable.isDataTable( '#contact' )) {
        let table = $('#contact').DataTable();
        table.destroy();
        setTable(true);
      }
      return () => {
        setRowsData([]);
        let dtable = $('#contact').DataTable();
        dtable.destroy(true);
        setTable(true);
      }
  }, []);
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#contact' ) ) {
  ...
  function loadDatatable() {
    ...
    let columnDefs = [
      ...
      },
      {
          ...
          "createdCell": function (td, cellData, rowData, row, col) {
              ...
              const contact_slug = rowData.id+'-'+username.replace(/\s+/g, '-').toLowerCase()
              ReactDOM.render(
                  <a target="_blank" style={{ cursor: 'pointer' }} href={"/contact/" + contact_slug + "/"}>
                      {cellData}
                  </a>
                  , td
              );
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              <Dropdown.Item onClick={() => history.push("/admin/contacts/"+rowData.id, "_blank") }><i className="flaticon-eye"></i> Views Details</Dropdown.Item>
              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/contacts/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#contact').on( 'page.dt', function () {             
      setPageNum($('#contact'), 'contacts', props);
    });
  }
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"contact"}>
                      <thead>
                      <tr>
