...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('news/'+selected[0]+'/', data).then(function (response) {
        ...
        <div className="kt-portlet__head kt-portlet__head--lg">
            <div className="kt-portlet__head-label">
              <span className="kt-portlet__head-icon">
                <i className="kt-font-brand flaticon2-open-text-book" />
              </span>
                {numSelected > 0 ? (
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/news/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
function parseRowData(rowData) {
    ...
    rowData.forEach(function(item, index){
        ...
        rowData[index]['date'] = moment(item.date).format('YYYY/MM/DD');
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abnews'),
    edit : userHasPermission('change_abnews'),
    delete : userHasPermission('delete_abnews'),
    view : userHasPermission('view_abnews'),
  });
  ...
  React.useEffect(() => {
      if($.fn.dataTable.isDataTable( '#news' )) {
        let table = $('#news').DataTable();
        table.destroy();
        setTable(true);
      }
      return () => {
          setRowsData([]);
          let dtable = $('#news').DataTable();
          dtable.destroy(true);
          setTable(true);
        }
  }, []);
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#news' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'news?format=datatables';
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
          ReactDOM.render(
          <a onClick={() => history.push("/admin/news/"+rowData.id) }>
          {cellData}
          </a>
              , td
          );
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/news/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#news').DataTable({
      ...
      serverSide:true,
      displayStart : getPageNum('news', props), 
      ajax: {
        url:url ,
        method: "GET",
    ...
    $('#news').on( 'page.dt', function () {             
      setPageNum($('#news'), 'news', props);
    });
  }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('news/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('news/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"news"}>
                      <thead>
                      <tr>
