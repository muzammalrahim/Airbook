...
import {list, post, patch, del, userHasPermission, API_URL, STATUSES} from "../../../crud/api";
...
const EnhancedTableToolbar = props => {
    ...
    function fetchCSVData() {
        ...
        list('engines', {records: 'all'}).then(response => {
            let related_models = ['type', 'user', 'model', 'manufacturer', 'category', 'owner', 'seller', 'primary_contact', 'current_location',]
                        ...
                        <div className="kt-portlet__head-actions">
                            {numSelected > 0 ? (
                                ...
                                <div className="btn-group">
                                    <Link to={"/admin/engine/asset/create"}
...
export default function EnhancedTable(props) {
    ...
    const [permission, setPermission] = React.useState({
        create: userHasPermission('add_abengines'),
        edit: userHasPermission('change_abengines'),
        delete: userHasPermission('delete_abengines'),
        view: userHasPermission('view_abengines'),
    });
    ...
    React.useEffect(() => {
        if (table && $.fn.dataTable.isDataTable('#engine')) {
            ...
            try {
                ...
                console.log('ignore - dt destroy', e);
              }
        }
    }, [rows]);
            ...
            createdCell: function(td, cellData, rowData, row, col) {
              ...
              ReactDOM.render(
                <a
                  ...
                  href={
                    "/engine/" +
                    engine_slug.replace(/ /g, "-").replace(/\//g, "-") +
                    "/"
                  }
                >
                  {cellData}
                        ...
                        ReactDOM.render(
                            <Dropdown className="kt-header__topbar-item" drop="down" alignLeft>
                                ...
                                <Dropdown.Menu
                                    ...
                                    {permission.view ? <Dropdown.Item onClick={() => history.push("/admin/engine/asset/" + rowData.id, "_blank") }>
                                        <i className="flaticon-eye"></i> Views Details
                                    </Dropdown.Item>:''}
                                    {permission.edit ? <Dropdown.Item
                                        onClick={() => history.push("/admin/engine/asset/" + rowData.id + "/edit")}><i
    ...
    function changeStatus(event, index) {
        ...
        let data = {isactivestatus: event.target.value, model: 'AbEngines', id: rowsData[index].id};
        setUser(index);
    ...
    function confirm(event) {
        if (action == "status") {
          ...
          patch("engines/" + rows[userIndex].id + "/", data).then(function(
            response
          ) {
          ...
          post("engines/" + rows[userIndex].id + "/promote/", data).then(
            function(response) {
