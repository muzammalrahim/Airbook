...
class Create extends React.Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			manufacturer:{label:'Select Manufacturer', value:0},
			manufacturers:[],
			selectedFile: null,
			previewFile: null,
			validated: false,
	...
	handleSubmit(event) {
		...
		post('type', this.state.types).then(
			(response) => {
				...
				this.props.data.history.push(this.state.action == 'save_new' ? this.clearForm(this.state.type+"-model-form"):"/admin/"+this.state.type+"/type");
		}).catch(error => { 
	...
	render() {
		const { selectedFile, types, manufacturer, manufacturers } = this.state;
		return (
			<Form
				noValidate
