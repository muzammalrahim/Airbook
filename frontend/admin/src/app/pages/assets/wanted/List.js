...
import { list, patch, del, post, userHasPermission, API_URL, STATUSES} from "../../../crud/api";
...
const EnhancedTableToolbar = props => {
  ...
  function fetchCSVData() {
    ...
    list('wanteds', {records:'all'}).then(response => {
      let related_models = ['user', 'type_0','model','manufacturer','primary_contact']
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/wanted/asset/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abwanteds'),
    edit : userHasPermission('change_abwanteds'),
    delete : userHasPermission('delete_abwanteds'),
    view : userHasPermission('view_abwanteds'),
  });
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              {permission.view ? <Dropdown.Item onClick={() => history.push("/admin/wanted/asset/"+rowData.id) }>
                    <i className="flaticon-eye"></i> Views Details
                </Dropdown.Item>:''}
              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/wanted/asset/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
  ...
  function confirm(event) {
    if (action == "status") {
      ...
      patch("wanteds/" + rows[userIndex].id + "/", data).then(function(
        response
      ) {
      ...
      post("wanteds/" + rows[userIndex].id + "/promote/", data).then(function(
        response
      ) {
