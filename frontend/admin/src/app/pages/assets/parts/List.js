...
import { list, patch, del, userHasPermission, API_URL, STATUSES} from "../../../crud/api";
...
const EnhancedTableToolbar = props => {
  ...
  function fetchCSVData() {
    ...
    list('parts', {records:'all'}).then(response => {
      let related_models = ['condition', 'release', 'owner','seller','primary_contact']
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/part/asset/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abparts'),
    edit : userHasPermission('change_abparts'),
    delete : userHasPermission('delete_abparts'),
    view : userHasPermission('view_abparts'),
  });
    ...
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
                ...
                const part_slug = rowData.id+'-'+name.replace(/\s+/g, '-').toLowerCase()
          ReactDOM.render(
          <a target="_blank" style={{ cursor: 'pointer' }} href={"/parts/" + part_slug + "/"}>
          {cellData}
          </a>
              , td
          );
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              <Dropdown.Toggle as={ActionsDropdownToggle} />
                  ...
                  {permission.view ? <Dropdown.Item onClick={() => history.push("/admin/part/asset/"+rowData.id) }>
                    <i className="flaticon-eye"></i> Views Details
                </Dropdown.Item>:''}
              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/part/asset/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
