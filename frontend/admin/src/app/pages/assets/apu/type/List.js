...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('type/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('type', {records:'all', type:type}).then(response => {
      let related_models = ['manufacturer']
      ...
      document.getElementById('type-csv-btn').click();
    })
  }
                  ...
                  <div className="dropdown dropdown-inline">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/"+type+"/type/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
                                  ...
                                  <ul className="kt-nav">
                                    ...
                                    <li className="kt-nav__item">
                                      <a className="kt-nav__link">
                                        <i className="kt-nav__link-icon la la-upload" />
                                        <CSVReader inputId="category-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbTypes').then((response)=>{
...
function updateTable(type) {
    const table = $('#'+type+'_type').DataTable();
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abtypes'),
    edit : userHasPermission('change_abtypes'),
    delete : userHasPermission('delete_abtypes'),
    view : userHasPermission('view_abtypes'),
  });
  ...
  React.useEffect(() => {
    if($.fn.dataTable.isDataTable( '#'+type+'_type' )) {
        let table = $('#'+type+'_type').DataTable();
        table.destroy();
        setTable(true);
      }
    return () => {
          setRowsData([]);
          let dtable = $('#'+type+'_type').DataTable();
          dtable.destroy(true);
          setTable(true);
    }
  }, []);
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#'+type+'_type' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'type?type='+type+'&format=datatables';
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
          ReactDOM.render(
              <a style={{ cursor: 'pointer' }} onClick={() => history.push("/admin/"+type+"/type/"+rowData.id) }>
                  {cellData}
                </a>
              , td
          );
                    ...
                    <Dropdown className="kt-header__topbar-item" drop="down" alignLeft>
                        <Dropdown.Toggle as={ActionsDropdownToggle} />
                            <Dropdown.Menu  className="dropdown-menu-fit dropdown-menu-anim dropdown-menu-top-unround">
                              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/"+type+"/type/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#'+type+'_type').DataTable({
      ...
      serverSide:true,
      displayStart : getPageNum(type+'_type', props),
      aoSearchCols: [
        null,
        null,
        null,
    ...
    $('#'+type+'_type').on( 'page.dt', function () {
        setPageNum($('#'+type+'_type'), type+'_type', props);
    });
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('type/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('type/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={type+"_type"}>
                      <thead>
                      <tr>
