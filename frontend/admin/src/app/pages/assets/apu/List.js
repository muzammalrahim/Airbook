...
const EnhancedTableToolbar = props => {
    ...
    function handleDelAll(event) {
        if (selected.length) {
            ...
            del('apuss/' + selected[0] + '/', data).then(function (response) {
    ...
    function fetchCSVData() {
        ...
        list('apus', {records: 'all'}).then(response => {
            let related_models = ['type', 'user', 'model', 'manufacturer', 'category', 'owner', 'seller',
                'current_location']
                        ...
                        <div className="kt-portlet__head-actions">
                            {numSelected > 0 ? (
                                ...
                                <div className="btn-group">
                                    <Link to={"/admin/apu/asset/create"}
...
export default function EnhancedTable(props) {
    ...
    const [permission, setPermission] = React.useState({
        create: userHasPermission('add_abapus'),
        edit: userHasPermission('change_abapus'),
        delete: userHasPermission('delete_abapus'),
        view: userHasPermission('view_abapus'),
    });
                        ...
                        ReactDOM.render(
                            <Dropdown className="kt-header__topbar-item" drop="down" alignLeft>
                                ...
                                <Dropdown.Menu
                                    ...
                                    {permission.view ? <Dropdown.Item onClick={() => history.push("/admin/apu/asset/" + rowData.id) }>
                                        <i className="flaticon-eye"></i> Views Details
                                    </Dropdown.Item>:''}
                                    {permission.edit ? <Dropdown.Item
                                        onClick={() => history.push("/admin/apu/asset/" + rowData.id + "/edit")}><i
    ...
    function changeStatus(event, index) {
        ...
        let data = {isactivestatus: event.target.value, model: 'AbApus', id: rowsData[index].id};
        setUser(index);
    ...
    function confirm(event) {
        if (action == "status") {
          ...
          patch("apus/" + rows[userIndex].id + "/", data).then(function(
            response
          ) {
          ...
          post("apus/" + rows[userIndex].id + "/promote/", data).then(
            function(response) {
                      ...
                      >
                        <thead>
                          <tr>
                            ...
                            <th data-data="serial_number">SN</th>
