...
function createDataArray(name, status, action ) {
...
function updateTable(newData) {
    const table = $('#aircraft_types').DataTable();
    table.clear();
...
function parseRowData(rowData) {
  ...
  if( Array.isArray(rowData) && rowData.length > 0){
    rowData.forEach(function(item, index){
        JsonArray.push(createDataArray(item.name, item.is_active,false));
    });
  }
  return JsonArray;
}
...
export default function EnhancedTable() {
  ...
  window.table = $('#aircraft_types');
  onInitialLoad();
  function onInitialLoad() {
    if(count === 0) {
      getAircraftTypes(null);
      setCount(count+1);
    }
  ...
  React.useEffect(() => {
      if ( $.fn.dataTable.isDataTable( '#aircraft_types' ) ) {
  ...
  function getAircraftTypes(search) {
  ...
  function handleSearch(event) {
    getAircraftTypes({search:event.target.value});
  }
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          <div className="kt-portlet__body">
            ...
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id="aircraft_types">
                      <thead>
                      <tr>
                          <th>Name</th>
                          <th>Type</th>
