...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('manufacturer/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('manufacturer', {records:'all', type:type}).then(response => {
      let related_models = ['country']
      ...
      document.getElementById('manufacturer-csv-btn').click();
    })
  }
              ...
              <span className="kt-portlet__head-icon">
                ...
                {numSelected > 0 ? (
                  ...
                  <h3 className="kt-portlet__head-title">
                   {type.charAt(0).toUpperCase() + type.slice(1)} Manufacturers
                  </h3>
                )}
            </div>
                  ...
                  <div className="dropdown dropdown-inline">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/"+type+"/manufacturer/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
                                  ...
                                  <ul className="kt-nav">
                                    ...
                                    <li className="kt-nav__item">
                                      <a className="kt-nav__link">
                                        <i className="kt-nav__link-icon la la-upload" />
                                        <CSVReader inputId="category-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbManufacturers').then((response)=>{
...
function updateTable(type) {
    const table = $('#'+type+'_manufacturer').DataTable();
...
function parseRowData(rowData) {
  if( Array.isArray(rowData) && rowData.length > 0){
    rowData.forEach(function(item, index){
      rowData[index].categories__name = '';
      if(Array.isArray(item.categories) && item.categories.length > 0){
        let cats = '';
        item.categories.forEach(function(category, indexInner){
          cats += category.name + ', ';
        });
        rowData[index].categories__name = cats.slice(0, -2);
      }
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abmanufacturers'),
    edit : userHasPermission('change_abmanufacturers'),
    delete : userHasPermission('delete_abmanufacturers'),
    view : userHasPermission('view_abmanufacturers'),
  });
  ...
  React.useEffect(() => {
    if($.fn.dataTable.isDataTable( '#'+type+'_manufacturer' )) {
        let table = $('#'+type+'_manufacturer').DataTable();
        table.destroy();
        setTable(true);
      }
    return () => {
          setRowsData([]);
          let dtable = $('#'+type+'_manufacturer').DataTable();
          dtable.destroy(true);
          setTable(true);
    }
  }, []);
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#'+type+'_manufacturer' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'manufacturer?type='+type+'&format=datatables';
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
          ReactDOM.render(
              <a style={{ cursor: 'pointer' }} onClick={() => history.push("/admin/"+type+"/manufacturer/"+rowData.id) }>
                  {cellData}
                </a>
              , td
          );
                    ...
                    <Dropdown className="kt-header__topbar-item" drop="down" alignLeft>
                        <Dropdown.Toggle as={ActionsDropdownToggle} />
                            <Dropdown.Menu  className="dropdown-menu-fit dropdown-menu-anim dropdown-menu-top-unround">
                              {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/"+type+"/manufacturer/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
    ...
    $('#'+type+'_manufacturer').DataTable({
      ...
      displayStart : getPageNum(type+'_manufacturer', props),
      ajax: {
        url:url ,
        method: "GET",
    ...
    $('#'+type+'_manufacturer').on( 'page.dt', function () {
        setPageNum($('#'+type+'_manufacturer'), type+'_manufacturer', props);
    });
  }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('manufacturer/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('manufacturer/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          <div className="kt-portlet__body">
            ...
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={type+"_manufacturer"}>
                      <thead>
                        <tr>
