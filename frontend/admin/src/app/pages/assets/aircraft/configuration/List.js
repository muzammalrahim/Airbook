...
const EnhancedTableToolbar = props => {
  ...
  function handleDelAll(event) {
    if(selected.length) {
      ...
      del('configuration/'+selected[0]+'/', data).then(function (response) {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('configuration', {records:'all'}).then(response => {
      let related_models = ['job_title', 'company']
      ...
      document.getElementById('configuration-csv-btn').click();
    })
  }
                  ...
                  <div className="dropdown dropdown-inline">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/aircraft/configuration/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm">
                                  ...
                                  <ul className="kt-nav">
                                    ...
                                    <li className="kt-nav__item">
                                      <a className="kt-nav__link">
                                        <i className="kt-nav__link-icon la la-upload" />
                                        <CSVReader inputId="configuration-csv-reader" cssClass="kt-nav__link-text" cssInputClass="d-none" label="Import" onFileLoaded={data => importData(data, 'AbConfigurations').then((response)=>{
...
function updateTable() {
    const table = $('#aircraft_configuration').DataTable();
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abconfigurations'),
    edit : userHasPermission('change_abconfigurations'),
    delete : userHasPermission('delete_abconfigurations'),
    view : userHasPermission('view_abconfigurations'),
  });
  let history = useHistory();
  
  React.useEffect(() => {
    if($.fn.dataTable.isDataTable( '#aircraft_configuration' )) {
    let table = $('#aircraft_configuration').DataTable();
    table.destroy();
    setTable(true);
  }
}, []);
  ...
  React.useEffect(() => {
      if ( table && $.fn.dataTable.isDataTable( '#aircraft_configuration' ) ) {
    ...
    function loadDatatable() {
      let url = API_URL+'configuration?format=datatables';
      let columnDefs = [
        ...
        },
        {
          ...
          "createdCell": function (td, cellData, rowData, row, col) {
            ReactDOM.render(
            <a onClick={() => history.push("/admin/aircraft/configuration/"+rowData.id) }>
            {cellData}
            </a>
                , td
            );
        ...
        columnDefs.push(
            {
              ...
              "createdCell": function (td, cellData, rowData, row, col) {
                ...
                {permission.edit ? <Dropdown.Item onClick={() => history.push("/admin/aircraft/configuration/"+rowData.id+"/edit") }><i className="flaticon2-edit"></i> Edit Details</Dropdown.Item>:''}
      ...
      $('#aircraft_configuration').DataTable({
        responsive: {
          details: {
        ...
        serverSide:true,
        displayStart : getPageNum('configuration', props),
        ajax: {
          url:url ,
          method: "GET",
      ...
      $('#aircraft_configuration').on( 'page.dt', function () {             
        setPageNum($('#aircraft_configuration'), 'configuration', props);
      });  
    }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('configuration/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('configuration/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"aircraft_configuration"}>
                      <thead>
                      <tr>
