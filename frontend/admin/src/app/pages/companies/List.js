...
const EnhancedTableToolbar = props => {
  ...
  function fetchCSVData() {
    setExpSpin('');
    list('companies', {records:'all'}).then(response => {
      let related_models = ['city','state','country']
        setExpSpin('d-none');
      ...
      document.getElementById('company-csv-btn').click();
    })
  }
        ...
        <div className="kt-portlet__head kt-portlet__head--lg">
            <div className="kt-portlet__head-label">
              <span className="kt-portlet__head-icon">
                <i className="kt-font-brand flaticon-buildings" />
              </span>
                {numSelected > 0 ? (
                ...
                <div className="kt-portlet__head-actions">
                      {numSelected > 0 ? (
                          ...
                          <div className="btn-group">
                            <Link to={"/admin/companies/create"} className="btn btn-sm btn-brand btn-elevate btn-icon-sm new-record">
...
function parseRowData(rowData) {
    ...
    rowData.forEach(function(item, index){
      ...
      rowData[index]['specialities__name'] = specialities;
...
export default function EnhancedTable(props) {
  ...
  const [permission, setPermission] = React.useState({
    create : userHasPermission('add_abcompanies'),
    edit : userHasPermission('change_abcompanies'),
    delete : userHasPermission('delete_abcompanies'),
    view : userHasPermission('view_abcompanies'),
  });
  ...
  React.useEffect(() => {
      ...
      return () => {
        setRowsData([]);
        let dtable = $('#companies').DataTable();
        dtable.destroy(true);
        setTable(true);
      }
  }, []);
  ...
  React.useEffect(() => {
    if (table && $.fn.dataTable.isDataTable( '#companies' ) ) {
  ...
  function loadDatatable() {
    let url = API_URL+'companies?format=datatables';
    let columnDefs = [
      ...
      },
      {
        targets: 2,
        "createdCell": function (td, cellData, rowData, row, col) {
            const company_name = rowData.name;
            const company_slug = rowData.id+'-'+company_name.replace(/\s+/g, '-').toLowerCase();
          ReactDOM.render(
          <a target="_blank" style={{ cursor: 'pointer' }} href={"/company/" + company_slug + "/"}>
          {cellData}
          </a>
              , td
          );
      ...
      columnDefs.push(
          {
            ...
            "createdCell": function (td, cellData, rowData, row, col) {
              ...
              {permission.view ? <Dropdown.Item onClick={() => history.push("/admin/companies/"+rowData.id) }><i className="flaticon-eye"></i> Views Details</Dropdown.Item> : ''}
    ...
    $('#companies').DataTable({
      ...
      serverSide:true,
      displayStart : getPageNum('companies', props),
      ajax: {
        url:url ,
        method: "GET",
    ...
    $('#companies').on( 'page.dt', function () {             
      setPageNum($('#companies'), 'companies', props);
    });
  }
  ...
  function confirm(event) {
    if(action == 'status') {
      ...
      patch('companies/'+rows[userIndex].id+'/', data).then(function (response) {
      ...
      del('companies/'+rows[userIndex].id+'/').then(function (response) {
        let rowsData = rows.filter((v,i) => i !== userIndex);
        ...
        <div className="kt-portlet kt-portlet--mobile">
          ...
          {permission.view ?   
            <div className="kt-section">
                <div>
                  <table className="table table-striped- table-bordered table-hover table-checkable" id={"companies"}>
                      <thead>
                      <tr>
                          ...
                          <th data-data="specialities__name">Speciality</th>
