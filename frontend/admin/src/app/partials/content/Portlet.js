...
export const PortletHeaderDesc = forwardRef(({ className, ...props }, ref) => (
  // eslint-disable-next-line jsx-a11y/heading-has-content
  <div
    {...props}
    ref={ref}
    className={clsx("kt-portlet__head-desc", className)}
  />
));
...
export const PortletHeader = forwardRef(
  (
    { icon, title, desc, toolbar, className, classNameHeader, sticky = false, labelRef, ...props },
    ref
  ) => {
    ...
    return (
      <div
        ref={ref}
        className={clsx("kt-portlet__head", classNameHeader)}
        style={
          !sticky
            ? undefined
      ...
      >
        {title != null && (
          ...
          >
            ...
            {/* Wrap string and fragments in PortletHeaderDesc */
            typeof desc === "string" || isFragment(desc) ? (
              <PortletHeaderDesc>{desc}</PortletHeaderDesc>
            ) : (
              desc
            )}
          </div>
