...
export async function getServerSideProps({ params }) {
  ...
  await axios.get(
    process.env.NEXT_PUBLIC_API_URL + `api/engines/${id[0]}/`,{params:{frontend: true}}
  ).then((response) => {
    data = response.data;
  })
  .catch((error)=> {
...
class EngineDetailPage extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.props.search({ type: 'engine', value: null, detail:true });
    if(this.state.engine.id)
       this.getData();
  }
  ...
  get_description = () => {
    const description_attrs = [
      'title','offer_for','availability',{owner:'name'}, {manager:'name'}, {seller:'name'}, {category:'name'}, {manufacturer:'name'},
      {type:'name'}, {model:'name'}, 'esn','lease_terms', 'exchange_terms', 'lsv_decription',
      {engine_model:'name'}, 'tso',''
    ];
  ...
  getData() {
    ...
    let { manufacturer } = this.state.engine;
    manufacturer = manufacturer !== null ? manufacturer.id : manufacturer
    let data = [
      { api: `engines/${id[0]}/related_engines/${manufacturer}/`, params: {frontend:true}, key: 'related' },
      { api: `engines/${id[0]}/promoted_engines`, params: {frontend:true}, key: 'promoted' },
  ...
  handleSubmit(e) {
    e.preventDefault();
    let {message, engine} = this.state;
    if(message){
      if(isAuthenticated(this.props.store)){
      this.setState({submitting:true});
      post('leads_send_message', {message:message, model:'Engine', model_id:engine.id}).then((response) => {
  ...
  getGraphData(id, model){
    list(`engines/${id}/analytics`,{model}).then((response)=>{
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps !== this.props) {
     this.setState({engine:this.props.data})
    }
  }
  componentDidMount() {
    ...
    if(engine.id){
      ...
      post('views', {viewable_id:engine.id, viewable_type:'App\\Engine'});
      this.getGraphData(engine.id, "Engine", this);
   }
...
jQuery(document).ready(function () {
  ...
  render() {
    let { engine, submitting, mainImageUrl, isAuthenticate, related, sideAdvert, topAdvert, showCarousel, promoted, likesByID, viewsByID, monthsByID, monthsByType, viewsByType, likesByType } = this.state;
    let title = `${engine?.manufacturer?.name ? engine.manufacturer.name :'' }, ${engine?.type?.name ? engine.type.name : ''}, ${engine?.model?.name ? engine.model.name: ''}, ${engine?.offer_for ? 'engine available for '+engine?.offer_for:''}`
    let description = `${engine?.manufacturer?.name ? engine.manufacturer.name :'' }, ${engine?.type?.name ? engine.type.name : ''}, ${engine?.model?.name ? engine.model.name: ''},${engine?.category?.name ? engine.category.name: ''}, ${engine?.offer_for ? 'engine available for '+engine?.offer_for:''}, ${engine?.country?.name ? 'current location '+engine.country.name : ''}.Browse more engine in Airbook ${engine?.category?.name ? engine.category.name:''} listing.`

    return (
      <>
        {showCarousel && engine.media.length > 0 && (
          <div
            id="focus"
          ...
          >
            ...
            <div className="img-setting">
              <Carousel
                ...
                infiniteLoop={true}
              >
                {engine.media.length > 0 &&
                  engine.media.map((media, index) => {
                    return (
                      <figure key={index} className="w-lightbox-figure">
                        <img
                          className="w-lightbox-group w-lightbox-image imageHeight"
        ...
        <Header
          ...
          data={{
            ...
            og_image: engine?.media?.length
              ? engine.media[0].original_file_name
              : null,
          }}
        />
            ...
            {engine.id ? (
              <>
                ...
                <div className="breadcrumb-block">
                  ...
                  <Link href="/engine/p/1">
                    <a className="breadcrumbs-link">Engines</a>
                  </Link>
                  <div className="b-arrow w-embed">
                    <svg
                      width={8}
                ...
                <div className="flex-wrapper">
                  <div className="flex-col-left">
                            ...
                            <div className="gal-info-block">
                              <div className="image-controls">
                                ...
                                <div className="image-count">{`${
                                  engine.media && engine.media.length
                                } Photos`}</div>
                              </div>
                    ...
                    <div className="details-data-block">
                      ...
                      {checkValue(engine.offer_for) ? (
                        <div className="data-block">
                          ...
                          <p className="data-value">{engine.offer_for}</p>
                        </div>
                      ) : (
                        ""
                      )}
                      {checkValue(engine.price) && engine.price >= 1 ? (
                        <div className="data-block">
                      ...
                      {checkValue(engine.offer_for) &&
                      ((engine.offer_for === "Lease" && engine.lease_terms) ||
                        (engine.offer_for === "Exchange" &&
                          engine.exchange_terms)) ? (
                        <div className="data-block">
                          <div className="data-label">Terms</div>
                          <p className="data-value">
                            {checkValue(engine.offer_for) &&
                            engine.offer_for === "Lease"
                              ? engine.lease_terms
                                ? engine.lease_terms
                                : "N/A"
                              : engine.offer_for === "Exchange"
                      ...
                      {formatDate(engine.availability, "availability") ? (
                        <div className="data-block">
                      ...
                      {checkValue(
                        engine.current_location && engine.current_location.name
                      ) ? (
                        <div className="data-block">
                          <div className="data-label">Location</div>
                          {checkValue(
                            engine.current_location &&
                              engine.current_location.flag
                          ) ? (
                            <div
                              className="svg-flag"
                              style={{
                                backgroundImage: `url(/static/media/site/flags/${engine.current_location.flag})`,
                              }}
                            />
                          ) : (
                            ""
                      ...
                      {checkValue(engine.owner && engine.owner.name) ? (
                        <div className="data-block">
                          <div className="data-label">Owner</div>
                          <p className="data-value">
                            {engine.owner && engine.owner.name}
                          </p>
                        </div>
                      ) : (
                        ""
                      )}
                      {checkValue(engine.seller && engine.seller.name) ? (
                        <div className="data-block">
                          <div className="data-label">Seller</div>
                          <p className="data-value">
                            {engine.seller && engine.seller.name}
                          </p>
                        </div>
                      ) : (
                        ""
                    ...
                    <div className="details-data-block">
                      ...
                      {checkValue(engine.category && engine.category.name) ? (
                        <div className="data-block">
                          <div className="data-label">Category</div>
                          <p className="data-value">
                            {engine.category && engine.category.name}
                          </p>
                        </div>
                      ) : (
                        ""
                      ...
                      {checkValue(engine.type && engine.type.name) ? (
                        <div className="data-block">
                      ...
                      {checkValue(engine.model && engine.model.name) ? (
                        <div className="data-block">
                      ...
                      {checkValue(engine.esn) ? (
                        <div className="data-block">
                          <div className="data-label">Serial #</div>
                          {isAuthenticate ? (
                            <p className="data-value">{engine.esn}</p>
                          ) : (
                    ...
                    <div className="details-data-block">
                      {checkValue(
                        engine.status ||
                          engine.cycle_remaining ||
                          engine.tso ||
                          engine.thrust_rating ||
                          engine.lsv_description
                      ) ? (
                        <div className="label-block">
                          <div className="ab-svg-icon details-page w-embed">
                            <svg
                      ...
                      {checkValue(engine.status) ? (
                        <div className="data-block">
                          ...
                          <p className="data-value">{engine.status}</p>
                        </div>
                      ) : (
                        ""
                      )}
                      {checkValue(engine.cycle_remaining) ? (
                        <div className="data-block">
                          ...
                          <p className="data-value">{engine.cycle_remaining}</p>
                        </div>
                      ) : (
                        ""
                      )}
                      {checkValue(engine.tso) ? (
                        <div className="data-block">
                          <div className="data-label">TSO</div>
                          <p className="data-value">{engine.tso}</p>
                        </div>
                      ) : (
                        ""
                      )}
                      {checkValue(engine.thrust_rating) ? (
                        <div className="data-block">
                          <div className="data-label">Thrust Rating</div>
                          <p className="data-value">
                            <span className="unit-span">
                              {engine.thrust_rating ? "lbs" : ""}
                            </span>
                          </p>
                        </div>
                      ) : (
                      ...
                      {checkValue(engine.lsv_description) ? (
                        <div className="data-block">
                          <div className="data-label">LSV Description</div>
                          <p className="data-value">{engine.lsv_description}</p>
                        </div>
                      ) : (
                        ""
                      )}
                    </div>
                    {checkValue(engine.description) ? (
                      <div className="details-data-block">
                        <div className="label-block">
                          <div className="ab-svg-icon details-page w-embed">
                            <svg
                        ...
                        <div className="data-block">
                          <p className="data-value">{engine.description}</p>
                        </div>
                      </div>
                    ) : (
                      ""
                    )}
                    {engine.attachments.length ? (
                      <div className="details-data-block">
                        <div className="label-block">
                          <div className="ab-svg-icon details-page w-embed">
                            <svg
                        ...
                        {engine.attachments.length > 0 &&
                          engine.attachments.map((attachment, index) => {
                            return (
                      ...
                      <div className="data-block charts">
                        ...
                        <div className="analytics-block">
                          {monthsByType.length > 0 && (
                            <Canvas
                              title={`All ${getTitle(engine.title)} on Airbook`}
                              labels={monthsByType}
                              views={viewsByType}
                              likes={likesByType}
                            />
                  ...
                  <div className="flex-col-right">
                    ...
                    <div className="sidebar-data-wrapper">
                      ...
                      {isAuthenticate ? (
                          ...
                          <div className="sidebar-publisher-info">
                            <div className="profile-block">
                              ...
                              <div className="sidebar-publisher-txt">
                                {engine.primary_contact &&
                                  engine.primary_contact.job_title &&
                                  engine.primary_contact.job_title.name}
                              </div>
                              <div className="member-info">
                                Member since {formatDate(engine.created_at)}
                              </div>
                              <Link
                                href={{ pathname: `/company/detail` }}
                                as={`/company/${
                                  engine.primary_contact &&
                                  engine.primary_contact.company &&
                                  engine.primary_contact.company.id +
                                    "-" +
                                    engine.primary_contact.company.name
                                }`}
                              >
                                <a
                                  target="_blank"
                        ...
                        <div className="locked-alert">Login to view</div>
                            ...
                            {checkValue(
                              engine.primary_contact &&
                                engine.primary_contact.country &&
                                engine.primary_contact.country.name
                            ) ? (
                    ...
                    <div className="sidebar-data-wrapper">
                      ...
                      <div className="share-flex">
                        <a
                          href={`https://www.linkedin.com/shareArticle?mini=true&url=${process.env.NEXT_PUBLIC_API_URL}engine/${this.props.params.detail}`}
                          target="_blank"
                        ...
                        <a
                          href={`https://www.facebook.com/sharer/sharer.php?u=${process.env.NEXT_PUBLIC_API_URL}engine/${this.props.params.detail}`}
                          target="_blank"
                          className="share-widget w-inline-block"
                        >
                          <div className="w-embed">
                        ...
                        <a
                          href={`https://api.whatsapp.com/send?text=${process.env.NEXT_PUBLIC_API_URL}engine/${this.props.params.detail}`}
                          target="_blank"
                          className="share-widget w-inline-block"
                        >
                          <div className="w-embed">
                      ...
                      <div className="sidebar-promoted-block">
                        <div className="label-block premium-block">
                          ...
                          <div className="premium-listings">
                            Promoted Engines listing
                          </div>
                        </div>
                        {promoted.length > 0 ? (
                          promoted.map((engine, index) => {
                            return (
                              <div
...
export default connect(null,site.actions)(withRouter(EngineDetailPage));
